...
timestamp, random nonce, and optional intended payee) using a CFP-held
secret key. The credential is the value embedded in URIs. It cannot be
forged without the CFP's secret, and any modification to the underlying
attributes invalidates it.
...
| `intended_payee`        | string | MAY  | AID or wildcard pattern (`utap:agent:<domain>:*`)   |
...
- `intended_payee`: Optional AID or wildcard pattern; use empty string (`""`) if absent
...
Verifiers MUST branch by token `version` when recomputing HMAC input
(`utap-0.1` without `intended_payee`; `utap-0.1.1` with `intended_payee`).
...
def mint_credential(cfp_secret, token_id, owner, amount, currency, timestamp, intended_payee=""):
    nonce = os.urandom(16)
    parts = [token_id, owner, amount, currency, timestamp, nonce.hex(), intended_payee or ""]
...
```javascript
async function mintCredential(cfpSecret, tokenId, owner, amount, currency, timestamp, intendedPayee = "") {
  const nonce = crypto.getRandomValues(new Uint8Array(16));
  const nonceHex = Array.from(nonce).map(b => b.toString(16).padStart(2, "0")).join("");
  const parts = [tokenId, owner, amount, currency, timestamp, nonceHex, intendedPayee || ""];
...
```python
def verify_credential(cfp_secret, credential, token_id, owner, amount, currency, timestamp, nonce, intended_payee=""):
    parts = [token_id, owner, amount, currency, timestamp, nonce, intended_payee or ""]
    credential_input = b"\x00".join(p.encode("utf-8") for p in parts)
...
If present, `intended_payee` binds a token to one PA identity (or a constrained
PA set using `utap:agent:<domain>:*`).

The CFP MUST enforce `intended_payee` at claim, hold, and transfer time. The
PA identity used for matching MUST come from the authenticated JWT context,
not from request body fields. On mismatch, the CFP MUST reject with HTTP 403
and `PAYEE_MISMATCH`.
...
```
https://{cfp-host}/pay?utap_version=0.1.1&utap_amount={amount}&utap_currency={currency}&utap_purpose={category}&utap_callback={callback_url}
```
...
```
https://cfp.example.com/pay?utap_credential=NEW&utap_version=0.1.1&utap_amount=1500.00&utap_currency=USD&utap_purpose=compute&utap_callback=https://provider.example.com/webhook/utap
```
...
- For HTTP integrations where URI alteration is not required, credential
  placement in headers or bodies SHOULD be preferred over query strings.
- If query-string transport is used, implementations MUST redact
  `utap_credential` in access logs, reverse proxies/WAF logs, traces,
  metrics labels, crash reports, and analytics sinks.
- Referrer policy controls MUST prevent outbound credential leakage via
  `Referer` headers.
...
8. If present, `intended_payee` is a valid AID or wildcard pattern.
...
For security decisions, the CFP MUST derive the presenting PA identity from
the authenticated JWT context, not from `presenting_agent` in the request body.
If `intended_payee` is set and does not match, the CFP MUST return HTTP 403
with `PAYEE_MISMATCH`.
...
For failed validation or claim checks, the CFP SHOULD minimize external error
surface differences (shape and timing) where feasible.
...
### 8.4.1 Atomic Claim (Normative)

To remove validate→hold race windows, CFP implementations MUST provide:
...
The CFP MUST execute claim atomically in one serializable transaction:
credential verification, token state checks, policy checks (including
`intended_payee`), transition to `HELD` bound to the authenticated PA, hold
expiry assignment, and audit record write. Concurrent claims for the same token
MUST result in exactly one success. Losing requests MUST return
`CLAIM_CONFLICT` or `TOKEN_STATE_CONFLICT`. Expired holds MUST return
`HOLD_EXPIRED`.
...
If `intended_payee` is set and does not match the authenticated PA, hold MUST
be rejected with HTTP 403 and `PAYEE_MISMATCH`.
...
If `intended_payee` is set and does not match the authenticated PA, transfer
MUST be rejected with HTTP 403 and `PAYEE_MISMATCH`.
...
| Scenario | CFP Action |
|----------|------------|
| Full tuple is new | Process normally. Store the result with a 24-hour TTL. |
| Full tuple exists, previous request succeeded | Return the cached result. Add `X-Idempotent-Replay: true` header. |
| Same `(agent_id, endpoint, idempotency_key)` but different payload hash | Reject with `400 INVALID_IDEMPOTENCY`. |
| Full tuple exists, previous request still processing | Return `409 Conflict` with `Retry-After` header. |
| Full tuple exists, previous request failed | Allow retry. Process as new request. |

The same key string from different agents MUST NOT collide.

### 11.4 Distinguishing Retries from Double-Spends

Idempotency and token state checks are complementary:

- Same idempotency tuple replay: return cached response.
- Same `(agent_id, endpoint, idempotency_key)` with different payload:
  reject with `INVALID_IDEMPOTENCY`.
- Same token with conflicting state transitions: reject with
  `TOKEN_STATE_CONFLICT` or `TOKEN_ALREADY_CLAIMED`.
...
token's core attributes (ID, owner, amount, currency, timestamp, nonce, intended payee) using
...
- **Token draining:** The compromised PA can accept any credential presented to
  it and immediately call `POST /cfp/v1/tokens/claim` (or validate+hold)
  followed by `POST /cfp/v1/tokens/{id}/transfer`, redirecting funds to an
  account it controls.
...
- **Credential harvesting:** If the PA receives credentials over weak-
  confidentiality transports (Section 14.5), it could stockpile credentials
  and batch-drain them before detection.

**Protocol-level mitigations:**

- **Recipient binding:** `intended_payee` constrains successful claim/hold/
  transfer to the authenticated PA.
...
- The CFP MUST validate JWTs on every request, including signature, issuer,
  and `iat`/`nbf`/`exp` checks with bounded leeway (Section 12.9).
...
2. **Atomic state transitions:** The CFP MUST use serialized access for token
   state changes. Two concurrent claim requests for the same token MUST result
   in exactly one success and one conflict (`CLAIM_CONFLICT` or
...
The CFP MUST enforce rate limiting on `/cfp/v1/tokens/validate` and
`/cfp/v1/tokens/claim` with controls for:

- per-agent limits,
- per-source-IP/subnet limits,
- burst and sustained windows.

On repeated failures, the CFP SHOULD apply progressive backoff and emit
security alerts. Rate-limited requests MUST return HTTP 429 with a
`Retry-After` header.
...
### 12.9 Time Authority and Clock Skew
...
- Security-critical ordering/checks MUST use server-side authoritative or
  monotonic time.
- Health checks MUST alarm and MAY fail closed if skew exceeds policy.

### 12.10 Validation Error Surface

For credential validation and claim failures, the CFP SHOULD minimize
externally distinguishable differences between malformed credentials, unknown
credentials, and non-transferable token states where feasible.

### 12.11 Security Profiles

UTAP defines three profiles: `STRICT`, `COMPAT`, and `HIGH_RISK_PROFILE`.
`STRICT` is the recommended default and requires recipient binding, atomic
claim, encrypted credential exchange, and credential redaction controls.
`COMPAT` allows temporary migration behavior (including tokens without
`intended_payee`) and MUST publish a migration deadline. `HIGH_RISK_PROFILE`
permits non-confidential channels only with TTL ≤ 120 seconds, immediate claim,
replay anomaly alerting, and explicit operator risk acknowledgement.

### 12.12 Optional Proof-of-Possession Profile

An optional `POP_CLAIM` profile MAY require the PA to include a signed nonce in
`POST /cfp/v1/tokens/claim`. The CFP verifies the signature against the PA
registered public key and verifies nonce freshness.
...
- **Agent-to-CFP communication** (minting, validation, claim, transfer, burn) MUST
...
- **Agent-to-agent credential exchange** (the payment URI or credential
  string) MUST use encrypted transport. The credential's tamper-proof property
  means integrity checks remain cryptographically bound to CFP verification.
...
- **Confidential transport REQUIRED:** Encrypted transport MUST be used.
- **Credential redaction REQUIRED:** Systems MUST redact `utap_credential` from
  logs, traces, metrics labels, crash reports, and analytics sinks.
...
| POST   | `/cfp/v1/tokens/claim`            | Yes  | Atomically validate and hold |
...
In all cases, the agent-to-CFP communication (validation, claim, transfer,
burn) continues to use the HTTP/JSON binding defined in Section 14.2. Profile
constraints are defined in Section 12.11.
...
- The message queue MUST provide encryption in transit. While credential
...
- gRPC channels between agents MUST use TLS.
...
- MQTT brokers MUST require TLS (port 8883).
...
```
https://cfp.example.com/pay?utap_credential=NEW&utap_version=0.1.1&utap_amount=1500.00&utap_currency=USD&utap_purpose=compute&utap_ref=gpu-quote-8821
```
...
{ "valid": true, "token_id": "550e8400-...", "status": "HELD" }
```
...
```json
{
  ...
  "properties": {
    ...
    "intended_payee": {
      "type": "string",
      "pattern": "^utap:agent:[a-z0-9.-]+:(\\*|[A-Za-z0-9._-]+)$",
      "description": "Optional recipient binding (exact AID or wildcard)"
    },
    "status": {
      "type": "string",
